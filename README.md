# hey, i'm Avani Bhartiya!

i like building things, breaking them, and then figuring out how they actually work.

currently a third year engineering student, trying to get better one project at a time.

---

### what i'm doing

* learning: data analytics 
* working on: website development for a NGO
* exploring: web dev / data science / ai / dbms

---

### stuff i've worked with

python • data • javascript • html • css • react • git • github • vscode • sql • figma

(not an expert in everything, but getting there)

---

### a couple of projects

* Dincharya-Your AI Planning Companion : Intelligent Scheduling and Planning Companion [https://github.com/hemant1904/dincharya]
* Krushimitra- Crop Recommendation and Prediction : Machine Leaning Model for Farmers [https://github.com/hemant1904/hemant1904-Krushimitra-Crop-Prediction-Analysis]
* Swasthya Seva- AI driven Public Health Chatbot: Smart India Hackathon Project
* CA Sahayaak: One stop solution for all CA related calculations [https://github.com/avanibhartiya/ca-portal]
---

### random

* i prefer learning by building instead of tutorials
* debugging > writing code
* love jigsaw puzzles (especially the ones with 1000 pieces)

---

### reach out

* linkedin: https://www.linkedin.com/in/avanibhartiya/ 
* email: [bhartiya.avani@gmail.com](mailto:bhartiya.avani@gmail.com)

---
##  GitHub Stats
![GitHub Streak](https://streak-stats.demolab.com/?user=avanibhartiya&theme=tokyonight)
---
